...
const char *WINDOWTITLE = { "Project 06 -- Alan Neads" };
...
Animate( )
{
	...
	int side = 1;
	float curHeight = 0.0;
	float increment = 0.02 + (sin(Time * 2.0 * 3.14159) / 50.0);
	for (int i = 0; i < NUMCURVES; i++) {
		float ratio = (float)(NUMCURVES - i) / (float)NUMCURVES;

		Curves[i].p0.x = side * ratio;
		Curves[i].p0.y = curHeight;
		Curves[i].p0.z = 0.0;
		curHeight += increment;

		Curves[i].p1.x = side * ratio * sin(Time * 2.0 * 3.14159);
		Curves[i].p1.y = curHeight;
		Curves[i].p1.z = 1.3333 * side * ratio;
		curHeight += increment;

		Curves[i].p2.x = side * -1.0 * ratio * sin(Time * 2.0 * 3.14159);
		Curves[i].p2.y = curHeight;
		Curves[i].p2.z = 1.3333 * side * ratio;
		curHeight += increment;

		Curves[i].p3.x = side * -1.0 * ratio;
		Curves[i].p3.y = curHeight;
		Curves[i].p3.z = 0.0;
...
Display( )
{
	...
	for (int i = 0; i < NUMCURVES; i++) {
		...
		for (int it = 0; it <= NUMPOINTS; it++)
		{
			...
			float hsv[3] = { atan2(x,z) * 180.0 / 3.14159, y, 1.0 };
			float rgb[3];
			HsvRgb( hsv, rgb );
...
InitMenus( )
{
	...
	int controllinemenu = glutCreateMenu(DoControlLinesMenu);
	glutAddMenuEntry("Off", 0);
	glutAddMenuEntry("On", 1);

	int controlpointmenu = glutCreateMenu(DoControlPointsMenu);
	glutAddMenuEntry("Off", 0);
	glutAddMenuEntry("On", 1);
